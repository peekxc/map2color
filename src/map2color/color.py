...
def is_rgb_seq(S) -> bool:
    ...
    if not valid_shape:
        ...
        valid_values = S.dtype == np.uint8 or (
            np.all(S.flatten() < 256) and np.all(S.flatten() >= 0)
        )
        return valid_values
...
class BokehColorPalette:
    ...
    def lookup(self, color_pal: str):
        ...
        if color_pal in self.palettes:
            ...
            assert basename in self.palettes, (
                f"Unable to find color palette '{basename}'"
            )
            base_palette = self.palettes[basename]
        return (
            base_palette[pal_sz]
            if pal_sz in base_palette
            else base_palette[list(base_palette.keys())[-1]]
        )
...
def hex2rgb(colors: Union[Iterable, str]):
    ...
    if isinstance(colors, str):  # int(hex_str[i:i+2], 16) for i in range(1,6,2)
        ...
        return np.frombuffer(
            bytes.fromhex("".join(colors).replace("#", "")), dtype=np.uint8
        ).reshape((n, d))
...
def color_mapper(
    color_pal: str = "viridis", lb: Optional[float] = 0.0, ub: Optional[float] = 1.0
) -> Callable:
    import bokeh
    ...
    ##
    def _color_map(
        x: Iterable, output: str = ["rgba", "hex"], strategy: str = ["bin", "lerp"]
    ) -> np.ndarray:
        if strategy == ["bin", "lerp"] or strategy == "bin":
            ...
            x_unit = color_values[ind] + rel[:, np.newaxis] * (
                succ_values - color_values[ind]
            )
...
def _transform_lerp(data: np.ndarray, palette: Sequence, bins: Sequence = None):
    ...
    bins = (
        np.linspace(np.min(data), np.max(data), len(palette)) if bins is None else bins
    )
    assert len(bins) == len(palette), (
        "The number of bins should match the size of the supplied color palette"
    )
...
def _transform_bin(data: np.ndarray, palette: Sequence, bins: Sequence):
    ...
    bins = (
        np.linspace(np.min(data), np.max(data), len(palette) - 1)
        if bins is None
        else bins
    )
    assert len(bins) == (len(palette) - 1), (
        f"The number of bins ({len(bins)}) should match the size of the supplied color palette ({len(palette)}) - 1"
    )
...
def _lerp_palette(palette: Sequence, size: int) -> Sequence:
    ...
    assert is_hex_seq(palette), (
        "Expects the color palette to be an array of hexadecimal strings"
    )
    N_COLORS = len(palette)
...
def map2hex(
    ...
    **kwargs,
    ...
    if isinstance(palette, str):
        ...
        assert isinstance(palette, Sequence) or isinstance(palette, np.ndarray), (
            "Invalid color palette given; must be a sequence or array of colors."
        )
    ...
    if interp == "bin":
        ...
        raise ValueError(
            f"Invalid interpolation strategy supplied '{str(interp)}'; should be one of 'bin' or 'lerp'."
        )
